# OUJI 彩妝／護膚分類尺寸統一及次分類互動 — Design QA（2026-09-22）

- Source truth：production 護膚頁 `category.html?cat=cleanser`，以及用戶提供嘅次分類截圖。
- Desktop 1440px：護膚主圖 `1440×360`、分類列 `1200×90`；彩妝修正後完全相同。
- Mobile 390px：護膚主圖約 `390×292.5`、分類列 `390×92`；彩妝修正後 `390×292.97`、`390×92`。
- 手機彩妝第一層五個分類圖案再縮細約 15%；次分類圖案由 36px 縮至 30px，按鈕高度、文字及間距保持不變。
- `?cat=base` 初次載入：次分類 hidden、佔位 0px；撳「底妝」後顯示粉底／氣墊／遮瑕；再撳一次收起。
- 撳「眼妝」會顯示眼影／眼線／睫毛膏／眉筆；次分類產品數量維持隱藏。
- 手機及桌面水平 overflow 0px；browser console 0 error。
- 本輪只完成本地預覽，尚未部署。
...
# 彩妝手機分類列收窄 — Design QA（2026-09-22）

- 390 × 844px 實測：主分類列由 92px 收窄至 74px，每格高度 68px。
- 分界線上下各留 8px，跟隨整列比例縮短；五格維持等寬及置中。
- Logo 可見高度約 47px，保留上一輪已確認大小，只移除多餘透明佔位及上下留白。
- 底妝細分類初始隱藏，點擊後正常顯示；頁面橫向溢出為 0。
